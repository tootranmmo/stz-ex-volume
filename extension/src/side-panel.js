...
class VolumeSidePanel {
    ...
    async startChecking() {
        ...
        this.addLog(`Bắt đầu ${this.keywords.length} từ (${this.totalBatches} batch)`, 'success');
    ...
    async processBatches(tabId) {
        while (this.currentBatch < this.totalBatches && this.isRunning) {
            ...
            this.addLog(`📦 Batch ${this.currentBatch + 1}/${this.totalBatches}: ${batchKeywords.length} từ`, 'info');
            ...
            try {
                ...
                if (this.currentBatch < this.totalBatches) {
                    ...
                    this.addLog(`⏳ Chờ ${delay}s...`, 'warning');
                    await this.sleep(delay * 1000);
                }
            } catch (error) {
                this.addLog(`❌ Batch ${this.currentBatch + 1}: ${error.message}`, 'error');
    ...
    completeChecking() {
        ...
        const message = `✅ Hoàn thành! ${this.currentSession.processedKeywords} từ trong ${duration}s`;
    ...
    stopChecking() {
        if (confirm('Dừng kiểm tra?')) {
            ...
            const message = `⏹ Dừng! ${this.currentSession.processedKeywords}/${this.currentSession.totalKeywords} từ`;
